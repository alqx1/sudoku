...
bool check_naked_singles(uint16_t sudoku[9][9], uint16_t possible[9][9]) {
    ...
    for (size_t i = 0; i < 9; i++) {
        for (size_t j = 0; j < 9; j++) {
            // Če ima celica že vrednost, pojdi na naslednjo iteracijo
            ...
            for (int num = 1; num < 10; num++) {
                // Če ima celica možnost, prištejemo ena k številu možnosti
                if (get_note(possible[i][j], num)) {
                    possibilities++;
                }
            }

            // Če ima celica le eno možnost, ji dodelimo vrednost možnosti
            if (possibilities == 1) {
...
void naked_single(
    ...
    add_new_value(sudoku, possible, selected, possiblity);
}
